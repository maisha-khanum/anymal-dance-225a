...
int main() {
    ...
    auto trunk_task = std::make_shared<SaiPrimitives::MotionForceTask>(robot, "hip_base", Affine3d::Identity());
    trunk_task->disableInternalOtg();
    trunk_task->setDynamicDecouplingType(SaiPrimitives::FULL_DYNAMIC_DECOUPLING);
    ...
    for (int i = 0; i < primary_control_links.size(); ++i) {        
        ...
        primary_tasks[primary_control_links[i]] = std::make_shared<SaiPrimitives::MotionForceTask>(robot, primary_control_links[i], compliant_frame);
	...
	for (int i = 0; i < secondary_control_links.size(); ++i) {        
        ...
        secondary_tasks[secondary_control_links[i]] = std::make_shared<SaiPrimitives::MotionForceTask>(robot, secondary_control_links[i], compliant_frame);
        ...
        secondary_tasks[secondary_control_links[i]]->setDynamicDecouplingType(SaiPrimitives::DynamicDecouplingType::FULL_DYNAMIC_DECOUPLING);
