...
int main() {
    ...
    std::map<std::string, std::shared_ptr<SaiPrimitives::MotionForceTask>> pose_tasks;
    ...
    for (int i = 0; i < control_links.size(); ++i) {        
        ...
        pose_tasks[control_links[i]] = std::make_shared<SaiPrimitives::MotionForceTask>(robot, control_links[i], compliant_frame);
        pose_tasks[control_links[i]]->disableInternalOtg();
        pose_tasks[control_links[i]]->setDynamicDecouplingType(SaiPrimitives::FULL_DYNAMIC_DECOUPLING);
    ...
    // posture task
    auto posture_task = std::make_shared<SaiPrimitives::JointTask>(robot);
